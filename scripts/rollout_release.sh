...
FRONTEND_IMAGE="${FRONTEND_IMAGE_REPOSITORY}:${FRONTEND_IMAGE_TAG}"
...
FRONTEND_CID="$(compose_frontend_container_id)"
OLD_IMAGE_ID="$(docker inspect "$APP_CID" --format '{{.Image}}' 2>/dev/null || true)"
OLD_FRONTEND_IMAGE_ID="$(docker inspect "$FRONTEND_CID" --format '{{.Image}}' 2>/dev/null || true)"
{
...
log "Pulling image $FRONTEND_IMAGE"
"${COMPOSE[@]}" pull "$FRONTEND_SERVICE" || true
...
FRONTEND_CID="$(compose_frontend_container_id)"
container_is_running "$FRONTEND_CID" || fail "Frontend service '$FRONTEND_SERVICE' is not running after rollout"
